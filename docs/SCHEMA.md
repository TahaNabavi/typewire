...
an unwritten clause: _…if your team uses zod_. Standard Schema removes it for
...
type Result<T> =
  { value: T; issues?: undefined } | { issues: ReadonlyArray<Issue> }
type Issue = {
...
**Standard JSON Schema v1** — the _introspection_ half, which matters here more
...
| What _shape_ is this schema?                        | graphql document generator, `generate openapi`, the tester's auto-input, `mock` | Standard **JSON** Schema |
...
export type AnySchema =
  StandardSchemaV1 | { parse(v: unknown): unknown; safeParse(v: unknown): any }
...
returned `issues` array, and `ValidationError` is constructed by _us_, from _our_
...
| Was                                                                       | Becomes                                             | Why                                                                                                 |
| ------------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `import { zBlob, zFile, makeRequestSchema } from "@tahanabavi/typefetch"` | `…/zod`                                             | These _construct_ zod schemas. Leaving them on the root entry means a Valibot app still bundles zod |
...
zero dependencies _and_ no opinion about your validator.
...
1. **Does `nestjs`'s query-param coercion survive vendor-neutrally?**
   ...
   attempt, but _enumerating union branches_ is introspection — it may need the
...
3. **Do we accept Standard Schema _contracts_ in the CLI's config?** `contracts`
