...
import { readFileSync, readdirSync, existsSync, mkdirSync, writeFileSync, copyFileSync, statSync } from "node:fs";
...
function findRepoRoot(from) {
  ...
  throw new Error("Could not locate the monorepo root (no pnpm-workspace.yaml found above " + from + ")");
}
...
const readText = (path) => (existsSync(path) ? normalize(readFileSync(path, "utf8")) : "");
...
function deriveCategory(pkg) {
  ...
  if (!isTransport && (kw.has("api-client") || kw.has("http") || kw.has("socket") || kw.has("websocket"))) {
    return "core";
  }
  ...
  if (isTransport || kw.has("encryption") || kw.has("middleware")) return "transport";
  if (kw.has("react") || kw.has("query") || kw.has("cache") || kw.has("hooks")) return "state";
  return "core";
}
...
function deriveTransports(pkg) {
  ...
  if (kw.has("grpc") || kw.has("grpc-web") || kw.has("connect") || kw.has("connect-rpc") || subpaths.includes("./grpc")) {
    found.add("grpc");
  }
  if (kw.has("websocket") || kw.has("socket.io") || kw.has("socket") || subpaths.includes("./socket")) found.add("ws");
  if (kw.has("http") || kw.has("rest") || kw.has("fetch")) found.add("http");

  // A server that serves any non-http wire also serves http.
  if (found.size > 0 && (kw.has("nestjs") || kw.has("backend"))) found.add("http");
...
function readmeTagline(md) {
  ...
  for (const block of body.split(/\n\s*\n/)) {
    ...
    if (text.length > 40 && !block.trim().startsWith("```") && !block.trim().startsWith("|")) {
...
function readDocs(pkgDir, relPath) {
  ...
  const pages = (manifest.pages ?? [])
    .map((page) => {
      const source = page.readme === true
        ? join(pkgDir, "README.md")
        : join(pkgDir, "docs", page.file ?? "");
      if (!existsSync(source)) return null;

      const full = readText(source);
      const body = page.sections ? sliceSections(full, page.sections, page.intro === true) : full;
      return {
        slug: page.slug,
        title: page.title,
        /** Repo-relative, so the page can link to its own source. */
        path: page.readme === true ? `${relPath}/README.md` : `${relPath}/docs/${page.file}`,
...
async function fetchNpmVersion(name) {
  ...
  try {
    ...
    return { version: typeof json.version === "string" ? json.version : null, checked: true };
  } catch {
...
function readRoadmap(rootReadme) {
  ...
  for (const line of (section[1] ?? "").split("\n")) {
    ...
    const item = { title: title.trim(), detail: rest.join(" — ").trim() || null };
...
const sizeBudgets = new Map(
  (readJson(join(REPO_ROOT, "size-budget.json")).budgets ?? []).map((b) => [b.package, b]),
);
...
const packages = (
  await Promise.all(
    packageDirs.map(async (dir) => {
      ...
      const published = npm.checked ? npm.version !== null : pkg.version !== "0.0.0";
      const pending =
        npm.version && compareVersionsDesc(pkg.version, npm.version) < 0 ? pkg.version : null;
      ...
      return {
        ...
        exports: Object.keys(pkg.exports ?? {}).filter((e) => e !== "./package.json"),
  ...
  : readdirSync(EXAMPLES_DIR)
      ...
      .map((dir) => {
        ...
        return {
          ...
          uses: Object.keys(pkg.dependencies ?? {}).filter((d) => d.startsWith("@tahanabavi/")),
        };
      })
...
const registry = {
  ...
  source: "derived from packages/*/package.json, their READMEs, size-budget.json and the root README",
