  ...
  return (
    <Container className="py-16">
      ...
      <header className="max-w-3xl">
        <p className="font-mono text-xs uppercase tracking-[0.18em] text-blue">// PLAYGROUND</p>
        <h1 className="mt-3 text-4xl font-extrabold tracking-tight">
          A real project, running on this page
        </h1>
        <p className="mt-4 text-lg leading-relaxed text-muted-foreground">
          This is not a diagram of TypeWire. It is{" "}
          <code className="font-mono text-fg">@tahanabavi/typefetch</code> in your browser, talking
          to route handlers on this site, both built from one contract object — and a switch that
          breaks the server so you can watch validation catch it.
        </p>
      </header>
      ...
      <div className="enter-group mt-10 grid grid-cols-1 gap-4 md:grid-cols-3">
        {FILES.map((file) => (
          ...
          >
            <p className="font-mono text-[11px] uppercase tracking-[0.14em]" style={{ color: file.tone }}>
              {file.role}
            </p>
            <p className="mt-1.5 truncate font-mono text-xs text-fg" title={file.path}>
              {file.path}
            </p>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{file.note}</p>
          </div>
        ))}
      </div>
      ...
      <div className="enter-group mt-10 grid grid-cols-1 gap-5 lg:grid-cols-[1.2fr_1fr]">
        <div className="min-w-0">
          ...
          <CodeBlock code={CONTRACT_SOURCE} filename="src/features/playground/contracts.ts" />
        </div>
        ...
        <div className="min-w-0 space-y-5">
          ...
          <Panel>
            <h3 className="text-base font-bold text-fg">What this page is honest about</h3>
            <ul className="mt-3 space-y-2.5 text-sm leading-relaxed text-muted-foreground">
              <li className="flex gap-2.5">
                <span aria-hidden className="mt-1.5 size-1.5 flex-none rounded-full bg-dim" />
                The fixture is a module-level array, shared by everyone and reset whenever the
                serverless instance recycles. A user you create may be visible to the next visitor,
                and may vanish a minute later.
              </li>
              <li className="flex gap-2.5">
                <span aria-hidden className="mt-1.5 size-1.5 flex-none rounded-full bg-dim" />
                Only the HTTP transport runs here. GraphQL, gRPC and WebSocket use the same contract
                object, but their packages are not published yet.
              </li>
              <li className="flex gap-2.5">
                <span aria-hidden className="mt-1.5 size-1.5 flex-none rounded-full bg-dim" />
                You cannot edit the contract from the page. Doing that honestly needs a type-checking
                compiler in the browser, and a version that only pretends to typecheck would
                undercut the one claim this project makes.
              </li>
            </ul>
          </Panel>
        </div>
