...
export function PackageCard({ pkg }: { pkg: PackageEntry }) {
  return (
    <Panel dashed={!pkg.published} className={pkg.published ? "" : "opacity-85"}>
      <div className="flex items-start justify-between gap-3">
        <Link href={`/packages/${pkg.slug}`} className="group font-mono text-sm">
          <span className="text-dim">@tahanabavi/</span>
          <span className="font-semibold text-fg group-hover:text-blue">{pkg.short}</span>
        </Link>
