...
import { CATEGORY_LABEL, packages, packagesByCategory, publishedPackages } from "@/lib/registry";
  ...
  return (
    <Container className="py-16">
      <JsonLd
        data={graph(
          collectionSchema(
            ...
            PATHS.PACKAGES,
            packages.map((pkg) => ({ name: pkg.npm, path: `/packages/${pkg.slug}` })),
          ),
          breadcrumbSchema([
            { name: "TypeWire", path: "/" },
      ...
      <header className="max-w-3xl">
        <p className="font-mono text-xs uppercase tracking-[0.18em] text-blue">// PACKAGES</p>
        <h1 className="mt-3 text-4xl font-extrabold tracking-tight">
          {packages.length} packages, one contract
        </h1>
        <p className="mt-4 text-lg text-muted-foreground">
          {publishedPackages.length} published on npm. Versions, sizes, dependencies and release
          history are derived from the monorepo at build time.
        </p>
      </header>
      ...
      <section className="mt-16">
        <h2 className="mb-4 font-mono text-xs uppercase tracking-[0.18em] text-dim">Compare</h2>
        <div className="overflow-x-auto rounded-xl border border-hair">
          <table className="w-full min-w-[820px] text-left text-sm">
            ...
            <tbody className="divide-y divide-hair">
              {packages.map((pkg) => (
                <tr key={pkg.slug}>
                  <td className="px-4 py-3 font-mono text-xs text-fg">{pkg.short}</td>
                  <td className="px-4 py-3">
                    <StatusChip published={pkg.published} version={pkg.version} />
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{CATEGORY_LABEL[pkg.category]}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
                    {pkg.sizeCeilingGzip ? `≤ ${gzipSize(pkg.sizeCeilingGzip)}` : "—"}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{pkg.releases.length}</td>
                </tr>
              ))}
            </tbody>
          </table>
