  ...
  return (
    <Container className="py-16">
      <JsonLd
        data={graph(
          collectionSchema(
            ...
            examples.map((example) => ({ name: example.slug, path: `/examples#${example.slug}` })),
          ),
          breadcrumbSchema([
            { name: "TypeWire", path: "/" },
      ...
      <header className="max-w-3xl">
        <p className="font-mono text-xs uppercase tracking-[0.18em] text-blue">// EXAMPLES</p>
        <h1 className="mt-3 text-4xl font-extrabold tracking-tight">Runnable, and self-asserting</h1>
        <p className="mt-4 text-lg text-muted-foreground">
          Every example builds against the packages in this repo rather than a published version, so
          it cannot drift from the source. Several are headless: they assert and exit, which is why
          they run in CI.
        </p>
      </header>
        ...
        {examples.map((example) => {
          ...
          <Panel key={example.slug}>
            <div className="flex items-baseline justify-between gap-3">
              ...
              <h2 id={example.slug} className="scroll-mt-24 font-mono text-lg font-bold text-fg">
                {example.slug}
              </h2>
              <a
      ...
      {examples.length === 0 && (
        <p className="mt-12 text-muted-foreground">No examples found in the workspace.</p>
      )}
    </Container>
  );
}
