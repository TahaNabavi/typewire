...
export type PackageCategory =
  'core' | 'transport' | 'server' | 'state' | 'devtools' | 'tooling'
...
export function packagesByCategory(): Array<{
  category: PackageCategory
  items: PackageEntry[]
}> {
...
export const documentedPackages = packages.filter(
  (p): p is PackageEntry & { docs: PackageDocs } =>
    p.docs !== null && p.docs.pages.length > 0
)
