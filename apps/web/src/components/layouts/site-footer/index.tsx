    ...
    <footer className="border-t border-hair py-14">
      <div className="mx-auto max-w-[1200px] px-6">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {footerNav.map((column) => (
            <div key={column.title}>
              <h3 className="font-mono text-xs uppercase tracking-[0.18em] text-dim">
                {column.title}
              </h3>
                ...
                {column.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      ...
                      link.href.startsWith('http') === true
        ...
        <div className="mt-12 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-hair pt-6 font-mono text-xs text-dim">
          ...
          <span
            className="ml-auto"
            title="Package data derived from the monorepo at build time"
          >
            registry built{' '}
