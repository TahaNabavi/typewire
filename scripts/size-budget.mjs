...
if (asJson) {
  ...
  console.log('\nGzipped size budget\n')
  console.log(
    `  ${name('PACKAGE')}${num('GZIP')}${num('BUDGET')}${num('USED')}`
  )
  ...
  for (const r of results) {
    if (!r.built) {
      console.log(
        `  ${name(r.package)}${num('—')}${num(r.maxGzip)}   not built`
      )
      continue
    }
...
if (update) {
  config.budgets = config.budgets.map((budget) => {
    ...
    return {
      ...budget,
      maxGzip: Math.ceil((result.gzip * HEADROOM) / 100) * 100,
    }
  })
  writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n')
  console.log(
    'size-budget.json rewritten — review the diff before committing.\n'
  )
  process.exit(0)
}
