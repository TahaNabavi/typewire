...
const { ApiClient, RichError, describeEndpoint } =
  await import('../packages/typefetch/dist/index.mjs')
const { graphqlTransport } = await import('../packages/graphql/dist/index.mjs')
const { grpcTransport, GrpcCode, encodeFrame, decodeFrames } =
  await import('../packages/grpc/dist/index.mjs')
...
globalThis.fetch = async (url, init) => {
  ...
  if (String(url).includes('graphql')) {
    return new Response(
      JSON.stringify({ data: { user: { id: '1', name: 'Ada' } } }),
      {
        status: 200,
...
await check(
  'graphql route generates a selection set from the schema',
  async () => {
...
await check(
  'describeEndpoint degrades rather than throwing on an unknown transport',
  () => {
    ...
    assert(
      route.target === '?',
      `expected the degraded target, got ${route.target}`
    )

    // …and resolves fully once the adapter is handed to it.
    const resolved = describeEndpoint(contracts.user.syncUser, [
      grpcTransport(),
    ])
    assert(
      resolved.protocol === 'gRPC',
      `expected gRPC, got ${resolved.protocol}`
    )
  }
)
