...
describe('socketConfigFromEnv', () => {
  ...
  it('supports any prefix', () => {
    expect(
      socketConfigFromEnv('VITE_SOCKET_', { VITE_SOCKET_URL: '/ws' })
    ).toEqual({ url: '/ws' })
  })
  ...
  it("treats only the literal string 'false' as disabling a flag", () => {
    expect(
      socketConfigFromEnv('S_', { S_AUTO_CONNECT: 'false' }).autoConnect
    ).toBe(false)
    expect(
      socketConfigFromEnv('S_', { S_AUTO_CONNECT: 'true' }).autoConnect
    ).toBe(true)
  })

  it('ignores a non-numeric numeric var rather than yielding NaN', () => {
    expect(socketConfigFromEnv('S_', { S_RECONNECTION_DELAY: 'soon' })).toEqual(
      {}
    )
  })
...
describe('resolveSocketConfig', () => {
  ...
  it('keeps an explicit false for autoConnect', () => {
    expect(
      resolveSocketConfig({ url: '/ws', autoConnect: false }).autoConnect
    ).toBe(false)
  })
})
