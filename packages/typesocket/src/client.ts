...
export class SocketClient<C extends SocketContracts> {
  ...
  constructor(
    ...
    if (options.onDisconnect)
      this.lifecycle.disconnect.add(options.onDisconnect)
    if (options.onConnectError)
      this.lifecycle.connectError.add(options.onConnectError)
  ...
  private buildEmitApi(
    ...
    return Object.defineProperties(emit, {
      queue: {
        value: (input: unknown) => this.queueFrame(eventId, event, def, input),
      },
      ...
      if (prepared.override && 'ack' in prepared.override) {
        const mocked =
          typeof prepared.override.ack === 'function'
            ? (prepared.override.ack as (i: unknown) => unknown)(
                prepared.payload
              )
