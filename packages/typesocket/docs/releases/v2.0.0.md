...
generates itself, validated on the way out _and_ on the way in.
...
`eventId` — `"chat.sendMessage"` — which is the _same_ `"module.endpoint"` shape
...
_and_ stored it for re-registration on `connect`. Because socket.io's emitter
...
_any_ frame on that event failed validation, so an unrelated malformed message
