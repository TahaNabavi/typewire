...
export function generateDocument(input: GenerateDocumentInput): string {
  ...
  const properties = (requestSchema.properties ?? {}) as Record<string, JsonSchema>;
  ...
  if (!root) {
    ...
    const selection = selectionFor(responseSchema, responseSchema, "response", new Set());
  ...
  const args = names.length
    ...
    : "";
  const selection = selectionFor(responseSchema, responseSchema, root, new Set());
