  ...
  const client = new ApiClient(
    {
      ...
      transports: [
        graphqlTransport({ url: 'https://api.test/graphql', ...config }),
      ],
      ...clientConfig,
    },
    contracts
...
describe('sending operations', () => {
  ...
  it("runs the client's middleware chain", async () => {
    ...
    client.use(async (ctx, next) => {
      seen.push(
        `${ctx.route?.transport}/${ctx.route?.operation}/${ctx.route?.target}`
      )
      return next()
    })
    ;(fetch as jest.Mock).mockResolvedValueOnce(
...
describe('errors', () => {
  ...
  it('maps extensions.code onto the shared kind taxonomy', async () => {
    ...
    ;(fetch as jest.Mock).mockResolvedValueOnce(
      gqlOk(null, [
        { message: 'login', extensions: { code: 'UNAUTHENTICATED' } },
      ])
    )
...
describe('contract checking at init()', () => {
  it('rejects a schema whose selection set cannot be generated', () => {
    ...
    expect(() => client.init()).toThrow(
      /Could not generate a document for "user\.get"/
    )
  })
    ...
    const custom = {
      user: {
        search: {
          ...
          operation: 'query',
          document:
            'query FindUsers($q: String!) { search(q: $q) { ... on User { id } } }',
          root: 'search',
