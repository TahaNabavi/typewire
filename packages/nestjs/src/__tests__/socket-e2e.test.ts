...
class ChatGateway implements OnGatewayConnection {
  @WebSocketServer() server!: { emit(event: string, ...args: unknown[]): unknown };
...
describe("typesocket gateways", () => {
  ...
  it("enforces a client->server event's contract permission server-side", async () => {
    const seen = new Promise<Record<string, unknown>>((resolve) => {
      ...
      (client as unknown as { socket: { on(e: string, cb: (p: never) => void): void } })
  ...
  it("rejects an inbound frame that fails the contract", async () => {
    ...
    (client as unknown as { socket: { emit(e: string, p: unknown): void } }).socket.emit(
      "chat.sendMessage",
      { text: "" },
    );
  ...
  it("catches an acknowledgement that violates the contract", async () => {
    ...
    const emit = client.modules.chat.drifted({ text: "hi" }).catch(() => undefined);
  ...
  it("refuses two events that collide on one wire name", () => {
    expect(() =>
      bindSocketContracts(
        defineSocketContracts({
          a: {
            one: { direction: "client->server", event: "shared", request: z.object({}) },
          },
          b: {
            two: { direction: "client->server", event: "shared", request: z.object({}) },
          },
        }),
      ),
