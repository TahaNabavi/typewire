...
const clientAesEncrypt = (v: string) =>
  CryptoJS.AES.encrypt(v, symKey.key as any).toString()
...
describe('crypto primitives — round trips', () => {
  it.each(['AES', 'DES', 'Base64'] as const)(
    '%s round-trips',
    async (method) => {
...
describe('encryptResponseData — produces what the client can decrypt', () => {
  const encryption = {
    method: 'AES' as const,
    response: { token: true, meta: { code: true } },
  }
...
describe('per-direction methods', () => {
  it('uses method.request / method.response independently', async () => {
    ...
    expect(
      CryptoJS.enc.Base64.parse(encrypted.b).toString(CryptoJS.enc.Utf8)
    ).toBe('B')
  })
})
