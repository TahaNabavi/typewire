...
the client, where an adapter is *registered* at the setup site rather than
...
values that only exist after the *client* decodes the body. Returning the wrong
...
do differently. What the server *does* owe them is the two headers a browser
...
It exempts the success wrapper *and* the error branch, so a failing exempt route
