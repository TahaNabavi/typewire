...
describe("query client seams", () => {
  describe("gate", () => {
    ...
    it("lets the gate supply a result without ever calling the endpoint", async () => {
      ...
      const endpoint = makeEndpoint("user.getUser", async () => ({ id: "server" }));
  ...
  describe("sources", () => {
    ...
    it("a later tree wins a collision, so the specific client is passed last", () => {
      const generic = makeEndpoint("user.getUser", async () => ({ id: "generic" }));
      const specific = makeEndpoint("user.getUser", async () => ({ id: "specific" }));

      const map = collectSources({ user: { getUser: generic } }, { user: { getUser: specific } });
