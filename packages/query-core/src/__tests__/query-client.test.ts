...
describe('QueryClient', () => {
  describe('fetchQuery', () => {
    ...
    it('accepts a typesocket event by its eventId', async () => {
      ...
      expect(
        client.cache.find({ endpointId: 'chat.sendMessage' })
      ).toHaveLength(1)
    })
  })
  ...
  describe('relations', () => {
    ...
    it('supports a resolver that reads variables and data', async () => {
      ...
      expect(
        client.cache.find({ endpointId: 'user.getUser' })[0]?.getState()
          .isInvalidated
      ).toBe(true)
    })
    ...
    it('merges per-observer invalidates with the declared relations', async () => {
      ...
      expect(
        client.cache.find({ endpointId: 'user.listUsers' })[0]?.getState()
          .isInvalidated
      ).toBe(true)
    })
