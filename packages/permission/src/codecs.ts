...
function bigintToBytesBE(value: bigint): number[] {
  if (value < 0n)
    throw new Error('[type-permission] cannot encode a negative bitfield')
  const out: number[] = []
  let x = value
  while (x > 0n) {
...
function b64urlToBytes(s: string): number[] {
  ...
  for (let i = 0; i < s.length; i += 4) {
    ...
    const c2 =
      s[i + 2] !== undefined ? B64URL_LOOKUP[s[i + 2] as string] : undefined
    if (c2 === undefined) break
    bytes.push(((c1 & 0x0f) << 4) | (c2 >> 2))
    const c3 =
      s[i + 3] !== undefined ? B64URL_LOOKUP[s[i + 3] as string] : undefined
