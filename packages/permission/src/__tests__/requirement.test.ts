...
describe('authorize — the contract `permission` key', () => {
  ...
  it('require: needs all listed flags', () => {
    expect(P.authorize(perms, { require: ['chat.VIEW_CHANNEL'] }).granted).toBe(
      true
    )
  ...
  it('any: needs at least one', () => {
    expect(
      P.authorize(perms, { any: ['chat.ATTACH_FILES', 'chat.SEND_MESSAGES'] })
        .granted
    ).toBe(true)
    const d = P.authorize(perms, {
      any: ['chat.ATTACH_FILES', 'guild.KICK_MEMBERS'],
    })
    expect(d.granted).toBe(false)
