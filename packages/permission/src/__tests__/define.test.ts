...
describe('definePermissions — validation', () => {
  ...
  it('throws on a non-integer or negative bit', () => {
    expect(() => definePermissions({ a: { X: { bit: -1 } } })).toThrow(
      /invalid bit/
    )
    expect(() => definePermissions({ a: { X: { bit: 1.5 } } })).toThrow(
      /invalid bit/
    )
  })
...
describe('checks', () => {
  ...
  it('hasAll / hasAny with vacuous edges', () => {
    ...
    expect(P.hasAll(perms, ['chat.VIEW_CHANNEL', 'chat.SEND_MESSAGES'])).toBe(
      true
    )
    expect(P.hasAll(perms, ['chat.VIEW_CHANNEL', 'chat.ATTACH_FILES'])).toBe(
      false
    )
    expect(P.hasAll(perms, [])).toBe(true) // vacuous truth
    expect(P.hasAny(perms, ['chat.ATTACH_FILES', 'chat.VIEW_CHANNEL'])).toBe(
      true
    )
  ...
  it('list() returns declaration order, missing() the gaps', () => {
    ...
    expect(
      P.missing(perms, ['chat.VIEW_CHANNEL', 'chat.ATTACH_FILES'])
    ).toEqual(['chat.ATTACH_FILES'])
  })
