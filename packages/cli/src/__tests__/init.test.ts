...
const featureSet = (...ids: FeatureId[]) => new Set<FeatureId>(["typefetch", ...ids]);
...
const paths = (plan: { root: string; files: Array<{ path: string }> }) =>
  plan.files.map((file) =>
    file.path.slice(plan.root.length + 1).split(/[\\/]/).join("/"),
  );
...
describe("buildPlan", () => {
  ...
  it("adds the provider only for React, and .tsx only for TypeScript", async () => {
    const react = await project(
      { dependencies: { react: "19.0.0" }, devDependencies: { typescript: "^5" } },
      { "tsconfig.json": "{}" },
    );
    const node = await project({ dependencies: { express: "4" } });

    expect(paths(buildPlan({ project: react, features: featureSet("query") }))).toContain(
      "typewire/provider.tsx",
    );
    expect(paths(buildPlan({ project: node, features: featureSet("query") }))).not.toContain(
      "typewire/provider.tsx",
    );
  });
  ...
  it("wires each selected transport into the client", async () => {
    ...
    expect(config).toMatch(/import \{ createClient, transports \} from "\.\/typewire\/client"/);
  ...
  it("reads the env var the way the framework actually does", async () => {
    ...
    expect(clientOf(buildPlan({ project: next, features: featureSet() }))).toContain(
    ...
    expect(clientOf(buildPlan({ project: vite, features: featureSet() }))).toContain(
      "(import.meta as { env?: Record<string, string | undefined> }).env?.VITE_API_URL",
    );
    expect(clientOf(buildPlan({ project: node, features: featureSet() }))).toContain(
  ...
  it("declares the socket section in the config only when asked for", async () => {
    ...
    expect(
      contentOf(buildPlan({ project: info, features: featureSet() }), "typewire.config.ts"),
  ...
  it('marks React entry points "use client" in a Next.js app', async () => {
    ...
    expect(contentOf(plan, "provider.tsx").startsWith('"use client";')).toBe(true);
    expect(contentOf(plan, "devtools.tsx").startsWith('"use client";')).toBe(true);
  });

  it("leaves the directive out of a plain React app, where it means nothing", async () => {
    const react = await project(
      { dependencies: { react: "19" }, devDependencies: { typescript: "^5", vite: "5" } },
      { "tsconfig.json": "{}" },
    );
  ...
  it("explains why the CLI client reads a different env var than the browser one", async () => {
    ...
    expect(
      contentOf(buildPlan({ project: next, features: featureSet() }), "typewire.config.ts"),
    ...
    expect(
      contentOf(buildPlan({ project: node, features: featureSet() }), "typewire.config.ts"),
...
describe("runInit", () => {
  ...
  it("writes nothing under --dry-run", async () => {
    ...
    await expect(readFile(join(root, "typewire.config.ts"), "utf8")).rejects.toThrow();
  });
  ...
  it("asks, and scaffolds exactly what was typed", async () => {
    await project(
      { name: "app", dependencies: { react: "19" }, devDependencies: { typescript: "^5" } },
      { "tsconfig.json": "{}" },
    );
  ...
  it("prints what it chose when it is not asking", async () => {
    ...
    expect(logs.join("\n")).toMatch(/What does this project need\?.*→.*Query layer/s);
  });
});
