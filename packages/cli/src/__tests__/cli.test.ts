...
describe("release-doc", () => {
  ...
  it("writes docs/releases/<version>.md", async () => {
    const result = await runReleaseDocCommand({ version: "v2.0.0", title: "Pluggable transports" });
  ...
  it("never overwrites an existing note without --force", async () => {
    ...
    await writeFile(join(root, "docs", "releases", "v2.0.0.md"), "written", "utf8");
    ...
    expect((await runReleaseDocCommand({ version: "v2.0.0" })).created).toBe(false);
    await expect(
      readFile(join(root, "docs", "releases", "v2.0.0.md"), "utf8"),
    ).resolves.toBe("written");

    expect((await runReleaseDocCommand({ version: "v2.0.0", force: true })).created).toBe(true);
  });
  ...
  it("exits 2 on bad usage, not 1", async () => {
    ...
    const error = await runReleaseDocCommand({ version: "next" }).catch((e: unknown) => e);
  ...
  it("honours --output-dir", async () => {
    const result = await runReleaseDocCommand({ version: "v3.0.0", outputDir: "./notes" });
