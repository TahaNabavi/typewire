...
describe('typewire.config', () => {
  ...
  describe('extends', () => {
    ...
    it('reports a cycle instead of hanging', async () => {
      await write(
        'a.config.ts',
        `export default { extends: "./b.config.ts", typefetch: { contracts: {} } };`
      )
      await write(
        'b.config.ts',
        `export default { extends: "./a.config.ts", typefetch: { contracts: {} } };`
      )
      await write(
        'typewire.config.ts',
        `export default { extends: "./a.config.ts", typefetch: { contracts: {} } };`
  ...
  describe('validation points at the offending key', () => {
    ...
    it('suggests the right spelling for a mistyped section', async () => {
      ...
      expect(warnings.join('\n')).toMatch(
        /Unknown top-level key "typeFetch"[\s\S]*Did you mean "typefetch"\?/
      )
    })
  ...
  describe('per-command requirements', () => {
    ...
    it('names the command that needs the section', async () => {
      const config = {
        ...
        lint: {},
        diff: {},
        projects: [{ name: 'default', implicit: true, lint: {}, diff: {} }],
      }
      ...
      await resolveTypeFetchClient(
        {
          ...
          test: {},
          createClient,
        } as unknown as ResolvedTypeFetchSection,
        'test',
...
describe('projects — several API surfaces in one repo', () => {
  ...
  it('selects all projects by default, and the named ones on request', async () => {
    ...
    expect(selectProjects(config, ['admin']).map((p) => p.name)).toEqual([
      'admin',
    ])
  ...
  it('merges projects through extends', async () => {
    ...
    expect(config.projects.map((p) => p.name).sort()).toEqual([
      'admin',
      'landing',
    ])
