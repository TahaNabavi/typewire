...
export async function runCli(argv = process.argv): Promise<void> {
  ...
  switch (parsed.command) {
    ...
    case "list": {
      ...
      for (const project of selectProjects(config, getCsvFlag(parsed, "project"))) {
    ...
    default: {
      ...
      for (const project of projects) {
        ...
        const reportPaths = await writeReports(report, section, resolved, project);
...
async function writeReports(
  ...
  const formats = options.formats ?? section.test.report?.formats ?? ["markdown"];
