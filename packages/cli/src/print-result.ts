...
import type { Contracts, EndpointDefZ, TransportAdapter } from "@tahanabavi/typefetch";
...
export function printEndpointList(
  ...
  console.log(
    `\n${COLOR.bold}TypeFetch endpoints${COLOR.reset}` +
      (options.project ? ` ${COLOR.dim}· project ${options.project}${COLOR.reset}` : "") +
      "\n",
  );
...
export function printReleaseDoc(path: string, created: boolean) {
  ...
  if (!created) {
    console.log(`${COLOR.dim}It already exists. Pass --force to replace it.${COLOR.reset}`);
  }
}
