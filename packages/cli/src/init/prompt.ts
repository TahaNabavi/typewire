...
function interactivePrompter(rl: Interface, write: (line: string) => void): Prompter {
  return {
    interactive: true,

    async text(question, initial) {
      const answer = (await rl.question(`${question} ${dim(`(${initial})`)} `)).trim();
    ...
    async multiselect(question, choices) {
      ...
      const preselected = choices
        .map((choice, index) => (choice.selected && !choice.locked ? index + 1 : 0))
        .filter(Boolean)
        .join(",");
...
function nonInteractivePrompter(): Prompter {
  return {
    ...
    async multiselect(question, choices) {
      const chosen = choices.filter((choice) => choice.selected || choice.locked);
      console.log(
