...
describe("grpcTransport — Connect JSON", () => {
  ...
  it("does not double the slash when baseUrl has a trailing one", () => {
    const { url } = transport.build(
      contextFor(getUser, { id: "u_1" }, { baseUrl: "https://api.example.com/" }),
    );
...
describe("grpcTransport — validation at init()", () => {
  ...
  it("names the endpoint when service is missing", () => {
    const broken = { ...getUser, service: undefined } as unknown as AnyEndpointDefZ;
...
describe("grpcTransport — binary grpc-web", () => {
  ...
  function binaryResponse(parts: Uint8Array[], status = 200, headers: Record<string, string> = {}) {
  ...
  it("decodes the message frame and honours an OK trailer", async () => {
    ...
    const res = binaryResponse([encodeFrame(message), trailerFrame("grpc-status: 0\r\n")]);
  ...
  it("fails a 200 whose trailer reports a non-OK status", async () => {
    ...
    const res = binaryResponse([trailerFrame("grpc-status: 5\r\ngrpc-message: gone\r\n")]);

    await expect(transport.decode(res, contextFor(getUser))).rejects.toMatchObject({
      kind: "not_found",
      message: "gone",
      status: 404,
    });
