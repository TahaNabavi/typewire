...
const DEFAULT_OPTIONS: Required<
  Pick<
    ApiTestRunnerOptions,
    'mode' | 'timeout' | 'concurrency' | 'stopOnFail' | 'includeDestructive'
  >
> = {
  mode: 'live',
  timeout: 10_000,
...
export class ApiTestRunner<C extends Contracts> {
  ...
  async run(): Promise<ApiTestReport> {
    ...
    for (const item of endpoints) {
      ...
      if (
        this.options.stopOnFail &&
        endpointResults.some((result) => result.status === 'failed')
      ) {
        break
      }
    }
    ...
    for (let index = 0; index < cases.length; index++) {
      ...
      if (testCase.skip) {
        results.push({
          ...
          durationMs: 0,
          skipReason:
            typeof testCase.skip === 'string' ? testCase.skip : 'Case skipped',
        })
        continue
      }
  ...
  private getEndpointSkipReason(
    ...
    if (testConfig?.destructive && !this.options.includeDestructive)
      return 'Destructive endpoint skipped'

    const tags = testConfig?.tags ?? []
    if (
      this.options.includeTags?.length &&
      !tags.some((tag) => this.options.includeTags!.includes(tag))
    ) {
      return 'Endpoint does not match includeTags'
    }

    if (
      this.options.excludeTags?.length &&
      tags.some((tag) => this.options.excludeTags!.includes(tag))
    ) {
  ...
  private async runCasePhase(
    ...
    try {
      ...
      const response = await this.callClient(
        moduleName,
        endpointName,
        parsedInput,
        testCase
      ...
      const expectedErrorStatus =
        normalized.status && expectedStatuses.includes(normalized.status)
  ...
  private async callClient(
    ...
    if (!fn)
      throw new Error(`Client method not found: ${moduleName}.${endpointName}`)
