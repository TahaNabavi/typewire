...
describe("registration", () => {
  ...
  it("runs the adapter's own contract check at init(), not on first call", () => {
    const broken = {
      chat: { send: { transport: "echo", request: z.object({}), response: z.object({}) } },
  ...
  it("catches a missing http field the same way", () => {
    const broken = {
      user: { get: { path: "/x", request: z.object({}), response: z.object({}) } },
    } as unknown as Contracts;
...
describe("describeEndpoint", () => {
  it("describes an http endpoint without a client", () => {
    expect(describeEndpoint(contracts.chat.fetchOne as AnyEndpointDefZ)).toEqual({
      protocol: "HTTP",
      operation: "GET",
      target: "/messages/:id",
    });
...
describe("capability warnings", () => {
  ...
  it("warns once when progress is asked of a transport that cannot report it", async () => {
    ...
    await client.modules.chat.send({ text: "a" }, { onDownloadProgress: () => {} });
    await client.modules.chat.send({ text: "b" }, { onDownloadProgress: () => {} });
