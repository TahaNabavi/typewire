...
describe('TypeFetch testing feature', () => {
  ...
  describe('createApiTestRunner - live and full modes', () => {
    it('runs live mode through ApiClient modules and creates a useful report', async () => {
      fetchMock
        .mockResolvedValueOnce(
          jsonResponse([
            { id: 'user-1', name: 'Taha', email: 'taha@example.com' },
          ])
        )
        .mockResolvedValueOnce(
          jsonResponse({
    ...
    it('runs full mode as schema + mock + live for endpoints with mockData', async () => {
      ...
      fetchMock.mockResolvedValueOnce(
        jsonResponse([{ id: 'real-user-1', name: 'Real User' }])
      )

      const client = new ApiClient(
        { baseUrl: 'https://api.test.com' },
        oneEndpointContracts
      )
      client.init()
      ...
      expect(report.results.map((item) => item.phase)).toEqual([
        'schema',
        'mock',
        'live',
      ])
  ...
  describe('setup and teardown', () => {
    it('runs setup before cases and teardown after cases', async () => {
      ...
      fetchMock.mockResolvedValueOnce(
        jsonResponse({ id: 'setup-user', name: 'Setup User' })
      )
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.test.com/users/setup-user',
        {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
          body: undefined,
