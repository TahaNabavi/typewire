...
describe('TypeFetchProvider / useQueryClient', () => {
  it('provides the client to the tree', () => {
    ...
    function Probe() {
      return (
        <span data-testid="same">{String(useQueryClient() === client)}</span>
      )
    }
...
describe('useQuery', () => {
  it('moves from loading to data', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('out')).toHaveTextContent('Taha')
    )
  })
  ...
  it('shares one request between two components on the same key', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('a')).toHaveTextContent('Taha')
    )
  ...
  it('surfaces an error without throwing', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('out')).toHaveTextContent('boom')
    )
  })
...
describe('useMutation', () => {
  ...
  it('refetches a related query through declared relations', async () => {
    ...
    render(<Probe />, { wrapper: wrapper(client) })
    await waitFor(() =>
      expect(screen.getByTestId('out')).toHaveTextContent('v1')
    )
    ...
    await waitFor(() =>
      expect(screen.getByTestId('out')).toHaveTextContent('v2')
    )
  })
})
