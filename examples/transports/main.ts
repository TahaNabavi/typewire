...
const sentQuery = String(
  (graphqlRequest.body as { query?: string }).query ?? ''
)
...
for (const failure of failures) {
  assert.equal(
    failure.kind,
    'not_found',
    'every wire must normalize to not_found'
  )
}
...
function describeRoute(
  id: 'user.getUser' | 'user.profile' | 'user.syncUser'
): string {
...
async function kindOf(call: () => Promise<unknown>): Promise<RichError> {
  try {
    ...
    assert.ok(
      ...
      'every transport must fail with a RichError'
    )
    return error
  }
}
