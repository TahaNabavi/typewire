...
const httpCalls = (label: string) =>
  bridge.getSnapshot().filter((e) => e.kind === 'start' && e.label === label)
    .length
...
assert.equal(
  httpCalls('user.getUser'),
  1,
  'concurrent reads must share one request'
)
...
const got = await download(
  { path: { id: stored.id } },
  {
    onDownloadProgress: (p) =>
      downloadTicks.push({ loaded: p.loaded, percent: p.percent }),
  }
)
...
assert.equal(
  got.size,
  stored.bytes,
  'the round trip should preserve every byte'
)
...
assert.ok(
  sources.has('http') && sources.has('ws'),
  'both transports in one log'
)
assert.ok(
